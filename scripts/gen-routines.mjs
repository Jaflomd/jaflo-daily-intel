...
4) Produce las 5 secciones (español, personalizado a Javier) y escribe data/${w.key}/$TODAY.json (**sobrescribe** si ya existe) con el contrato (ver prompts/${w.n}-${w.key}.md):
...
5) Renderiza, recopila favoritos y publica:
   node scripts/build.mjs ${w.key} $TODAY     # HTML del dossier + MD estrellable (stars/${w.key}/)
...
6) Reporta: dominio, #ítems reales, si quedó delgado, y el link ${PAGES}
